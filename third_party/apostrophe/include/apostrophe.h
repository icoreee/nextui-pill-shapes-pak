...
int            ap_reload_status_assets(void);
...
void           ap_draw_button_pill(int x, int y, int w, int h, ap_color c);
...
int ap_reload_status_assets(void) {
    ...
    if (!tex) {
        ap_log("Warning: could not create status asset texture: %s", asset_path);
        return AP_ERROR;
    }

    SDL_SetTextureBlendMode(tex, SDL_BLENDMODE_BLEND);
    if (ap__g.status_assets) SDL_DestroyTexture(ap__g.status_assets);
    ap__g.status_assets = tex;
    ap__g.status_asset_scale = scale;
    ap_log("Reloaded status assets: %s", asset_path);
    return AP_OK;
}
...
void ap_draw_button_pill(int x, int y, int w, int h, ap_color c) {
    /* Match NextUI GFX_blitButton/GFX_blitButtonGroup:
       keycaps use ASSET_BUTTON at (1,33,20,20), not ASSET_WHITE_PILL. */
    if (ap__g.status_assets) {
        int s = ap__g.status_asset_scale;
        int sprite_x = 1 * s;
        int sprite_y = 33 * s;
...
static void ap__footer_draw_item(int *cx, int btn_y, int inner_h, int btn_margin,
    ...
    ap_draw_button_pill(*cx, btn_y, btn_pill_w, inner_h, ap__g.theme.highlight);
